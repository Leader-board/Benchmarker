/* Similar to the benchmarker MT version except that we run in batches of N threads, where N is the number of supported threads on the CPU.
The objective is to test and analyse the effects of context-switching, since in this case each thread will be able to run at full speed
without having to context-switch between all the other number_of_threads. If number_of_thread % N != 0, then number_of_thread % N are run in the end.
*/
...
int main()
{
	...
	auto begin = chrono::high_resolution_clock::now();
	// run NUMBER_OF_THREADS/num_threads instead at one go
